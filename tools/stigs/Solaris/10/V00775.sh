...
# Handle symlinks
directory="$(get_inode ${directory})"

# Ensure ${directory} exists
if [ ! -d ${directory} ]; then
  [ ${verbose} -eq 1 ] && print "'${directory}' doesn't exist"
  exit 1
fi
...
if [ ${change} -eq 1 ]; then
  ...
  # Print friendly message regarding validation
  [ ${verbose} -eq 1 ] && print "Setting permissions '${directory}'"
...
# Print friendly message regarding validation
[ ${verbose} -eq 1 ] && print "Obtained permissions for '${directory}'"

if [ ${perms} -ne ${perm} ]; then
