...
# Ensure we are working on a global zone if ${global} = 1
zone="$(zonename)"
if [ ${global} -eq 1]; then
  usage "STIG ID '${stigid}' only applies to global zones" && exit 1
fi
...
if [ ${change} -eq 1 ]; then
  ...
  # Backup the file
  cp ${file} ${file}.${author}-$(gen_date) || usage "Failed to create backup" && exit 1
  ...
  # Create & set permissions on tempoary file
  ...
  sed -e "s/^${key}=.*/${setting}/g" ${file} > ${tfile} || usage "Failed to make requested change" && exit 1

  # Print friendly message regarding temporary file permissions
  [ ${verbose} -eq 1 ] && print "Set '${settings}' in '${tfile}'"

  # Move ${tfile} into ${file}
  mv ${tfile} ${file} || usage "Failed to restore '${tfile}' to '${file}'" && exit 1
...
[ ${verbose} -eq 1 ] && print "Validating on '${file}' according to STIG ID '${stigid}'"

# Get ${settings} from ${file}
haystack="$(grep "${settings}" ${file})"
