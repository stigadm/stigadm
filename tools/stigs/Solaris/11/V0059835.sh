...
for inode in ${files[@]}; do
  ...
  # Extract possible LD_LIBRARY_PATH from ${inode} to an array as ${haystack[@]}
  haystack=( $(nawk '$0 ~ /LD_LIBRARY_PATH=/{if ($0 ~ /LD_LIBRARY_PATH=/){split($0, obj, "=");if(obj[2] ~ /;/){split(obj[2], fin, ";")}else{fin[2]=obj[2]}print fin[2]}}' ${inode} 2>/dev/null) )
